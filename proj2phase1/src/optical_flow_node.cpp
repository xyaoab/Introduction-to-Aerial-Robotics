...
cv::Size imageSize;
vector<cv::Point2f> points,prev_points(26*21),good_pts, good_prev_pts;
vector<uchar> status;
vector<float> err;
...
void imageCallback(const sensor_msgs::Image::ConstPtr &image_msg) {
    ...
    cv::Mat H = cv::findHomography(good_prev_pts, good_pts, 8 ,1,mask);
    ...
    if(!mask.empty())
    {
       for (int i=0;i<good_pts.size();i++)
        {   double yy=pow((good_prev_pts[i].y-good_pts[i].y),2.0);
            ...
            if(mask.at<double>(0,i) && sqrt(xx+yy) <20.0)
            {
